(function () {
    ...
    function ObjectMap(opts) {
        ...
        this._mirror_for_adding = opts.mirror_for_adding || null;
        this._mirror_for_removing = opts.mirror_for_removing || null;
    }
    ...
    ObjectMap.prototype.contains = function (id) {
        return this._map.hasOwnProperty(id);
    };

    ObjectMap.prototype.add = function (el) {
        this._add(el, null);
    };

    ObjectMap.prototype._add = function (el, sourceMap) {
        this._map[el[this._idField]] = el;
        this._mirror_for_adding && this._mirror_for_adding != sourceMap && this._mirror_for_adding._add(el, this);
    };

    ObjectMap.prototype.remove = function (id) {
        this._remove(id, null);
    };

    ObjectMap.prototype._remove = function (id, sourceMap) {
        if (this.contains(id)) {
            delete this._map[id];
            this._mirror_for_removing && this._mirror_for_removing != sourceMap && this._mirror_for_removing._remove(id, this);
        }
    };
    ...
    ObjectMap.prototype.each = function (callback, context) {
        Object.keys(this._map).forEach(function (id) {
            callback.call(context, this._map[id]);
        }, this);
    };

    ObjectMap.prototype.map = function (callback, context) {
        return Object.keys(this._map).map(function (id) {
            return callback.call(context, this._map[id]);
        }, this);
    };

    ObjectMap.prototype.every = function (callback, context) {
        return Object.keys(this._map).every(function (id) {
            return callback.call(context, this._map[id]);
        }, this);
    };

    ObjectMap.prototype.some = function (callback, context) {
        return Object.keys(this._map).some(function (id) {
            return callback.call(context, this._map[id]);
        }, this);
    };

    ObjectMap.prototype.to_array = function () {
        return Object.keys(this._map).map(function (id) {
            return this._map[id];
        }, this);
    };

    ObjectMap.prototype.to_arr = ObjectMap.prototype.to_array;

    ObjectMap.prototype.set_mirror_for_adding = function (mirror_for_adding) {
        this._mirror_for_adding = mirror_for_adding;
    };

    ObjectMap.prototype.set_mirror_for_removing = function (mirror_for_removing) {
        this._mirror_for_removing = mirror_for_removing;
    };
