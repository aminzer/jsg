(function () {
    ...
    var Hash = function () {
        var self = function (object) {
            this._object = _is_object(object) ? object : {};
        };
        ...
        self.prototype.add = function (key, value, overwrite) {
            if (overwrite || _is_undefined(this._object[key])) {
        ...
        self.prototype.add_defaults = function (object, overwrite) {
            Object.keys(object).forEach(function (key) {
                this.add(key, object[key], overwrite);
            }.bind(this));
            return this;
        };

        self.prototype.merge = function (object) {
            return this.add_defaults(object, true);
        };

        ['add', 'set', 'add_defaults', 'merge'].forEach(function (methodName) {
            self[methodName] = function (/* arguments... */) {
                var object = arguments[0],
                    methodParams = Array.prototype.slice.call(arguments, 1),
                    proxy = new self(object);
                return proxy[methodName].apply(proxy, methodParams).to_obj();
            }
        });
